...
export default function WireFeed() {
  ...
  const handleImport = async (itemId: string) => {
    ...
    try {
      const response = await vortex.wire.importToStory(itemId, currentStation.id);
    ...
    <div className="max-w-7xl mx-auto p-4 md:p-8 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
        <div>
          ...
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em]">Real-time Global Inbound Aggregate</p>
        </div>
        <button
          onClick={handleFetchAll}
          disabled={fetching}
          className="bg-slate-900 text-white px-10 py-4 rounded-[1.5rem] font-black uppercase tracking-widest text-[11px] hover:bg-slate-800 transition-all shadow-2xl active:scale-95 disabled:opacity-50 flex items-center gap-3"
        >
          {fetching ? (
            <>
      ...
      <div className="flex gap-2 mb-10 flex-wrap bg-white p-2 rounded-[2rem] shadow-sm border border-slate-100 inline-flex">
        <button
          onClick={() => setSelectedService('')}
          className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
            !selectedService ? 'bg-primary-600 text-white shadow-xl shadow-primary-600/20' : 'text-slate-400 hover:text-slate-600 hover:bg-slate-50'
          }`}
        >
          Aggregated Source
        </button>
        {services.map((service) => (
          <button
            ...
            className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${
              selectedService === service.slug ? 'bg-primary-600 text-white shadow-xl shadow-primary-600/20' : 'text-slate-400 hover:text-slate-600 hover:bg-slate-50'
            }`}
          >
            {service.name}
          </button>
      ...
      {loading ? (
        <div className="text-center py-40">
          ...
          <p className="text-[11px] font-black text-slate-300 uppercase tracking-[0.5em]">Querying Satellite Gateway...</p>
        </div>
      ) : (
            ...
            <div key={item.id} className="bg-white p-10 rounded-[3rem] shadow-sm hover:shadow-2xl border border-slate-100 transition-all flex flex-col md:flex-row justify-between items-start md:items-center gap-10 group">
              <div className="flex-grow">
                ...
                <div className="flex items-center gap-8">
                   <a href={item.link} className="text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-slate-900 flex items-center gap-2 border-b-2 border-slate-50 pb-1">
                   ...
                   {item.author && <span className="text-[9px] font-bold text-slate-300 uppercase tracking-widest italic">Source: {item.author}</span>}
                </div>
              </div>
              <div className="shrink-0 flex flex-col items-end">
                <button
                  onClick={() => handleImport(item.id)}
                  className="px-10 py-4 bg-primary-600 text-white text-[11px] font-black uppercase tracking-widest rounded-2xl hover:bg-primary-500 transition-all shadow-xl shadow-primary-600/30 active:scale-95"
                >
                  Import to Archive
                </button>
              </div>
          ...
          {items.length === 0 && (
            <div className="text-center py-40 bg-white rounded-[4rem] border-4 border-dashed border-slate-50">
              <div className="w-24 h-24 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-8">
                <svg className="w-12 h-12 text-slate-200" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.111 16.404a5.5 5.5 0 017.778 0M12 20h.01m-7.08-7.071c3.904-3.905 10.236-3.905 14.14 0M1.344 6.637c6.072-6.073 15.24-6.073 21.312 0" /></svg>
              </div>
              <p className="text-[11px] font-black text-slate-300 uppercase tracking-[0.5em]">No Intelligence Nodes available in this channel.</p>
            </div>
          )}
        </div>
      )}
